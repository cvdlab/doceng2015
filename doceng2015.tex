...
A syntax validator is provided by \emph{indoor.io}, but the commercial nature of this project limits the number of tools available to deal with this format.

\section{Advances on cartographics documents}\label{advances-on-cartographics-documents}

\textbf{HIJSON} (\textbf{H}ierarchical \textbf{I}ndoor \textbf{JSON}) is
based on GeoJSON. It reveals at least three major enhancements above 
the currently most adopted indoor cartographic document formats: it exposes a 
\emph{hierarchical structure}, uses \emph{metric local coordinate system} and accepts 
\emph{semantic extensions}.

\subsection{Hierarchical structure}\label{hierarchical-structure}

HIJSON organizes its elements in a hierarchical structure, where every element represent a
potential container for other elements, unlike other formats like GeoJSON or IndoorJSON where all the elements are disposed in a linear structure. This structure enables a clear
and logical organization of the elements inside the structure, and at
the same time make it possible to use a relative, local, metric coordinates system.
